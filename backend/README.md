# Mindease — FastAPI + PostgreSQL Backend
...
```
DATABASE_URL=postgresql+psycopg2://postgres:YOUR_PASSWORD@localhost:5432/Mindease
```
...
2. 🆕 Create the `Mindease` database (if it doesn't exist)
...
### `database "Mindease" does not exist`
- Run `python run_server.py` — it creates the database automatically
- Or manually: `psql -U postgres -c "CREATE DATABASE Mindease;"`

### Tables not being created
- Ensure `DATABASE_URL` points to the `Mindease` database (not `postgres`)
