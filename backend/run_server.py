"""
run_server.py — Mindease auto-startup script.

Steps performed automatically:
  1. Load environment variables from .env
  2. Connect to PostgreSQL server (default connection: postgres / postgres @ localhost:5432)
  3. Create the 'Mindease' database if it does not already exist
...
Equivalent manual steps:
    createdb -U postgres Mindease          # step 3
...
log = logging.getLogger("Mindease.startup")
...
DATABASE_URL: str = os.getenv(
    "DATABASE_URL",
    "postgresql+psycopg2://postgres:postgres@localhost:5432/Mindease",
)
...
def ensure_database_exists(db_url: str) -> None:
    ...
    try:
        ...
        target_db = parsed.database  # e.g. "Mindease"
    ...
    try:
        ...
        log.error(
            ...
            "        psql -U postgres -c \"CREATE DATABASE Mindease;\"\n",
            exc,
        )
        sys.exit(1)
...
def start_server() -> None:
    ...
    log.info("🚀  Starting Mindease API on http://%s:%s", host, port)
