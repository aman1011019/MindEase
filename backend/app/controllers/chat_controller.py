...
def handle_send_message(db: Session, message: str, mood: str) -> ChatResponse:
    ...
    # Pass to ai_service.
    # The frontend sends mood as a plain string ("sad", "happy", …), not an emoji,
    # so we pass it as the emoji arg — ai_service falls back gracefully when the
    # emoji isn't recognised, so we inject the mood directly via a sentinel prefix.
    ai_result = ai_service.handle_chat(message, mood, context, mood_override=mood)
